function DenoisedImg = FAST_NLM_III(NoisyImg, ClearImg,PatchSizeHalf,WindowSizeHalf,Sigma)
% -------------------------------------------------------------------------
...
Z = M;
% Pad noisy image to avoid Border Issues
...
PaddedC = padarray(ClearImg,[WindowSizeHalf,WindowSizeHalf],'symmetric','both');
% Main loop
...
%         SqDist = Sd(PatchSizeHalf+1:end-PatchSizeHalf,PatchSizeHalf+1:end-PatchSizeHalf)+Sd(1:end-2*PatchSizeHalf,1:end-2*PatchSizeHalf)-Sd(1:end-2*PatchSizeHalf,PatchSizeHalf+1:end-PatchSizeHalf)-Sd(PatchSizeHalf+1:end-PatchSizeHalf,1:end-2*PatchSizeHalf);       
        ...
        Sd = integralImgSqDiff(PaddedV, PaddedC, dx,dy);
        ...
        c = PaddedC((WindowSizeHalf+1+dx):(WindowSizeHalf+dx+Height),(WindowSizeHalf+1+dy):(WindowSizeHalf+dy+Width));
