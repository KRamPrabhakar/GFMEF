...
%     I_Cb128_1 = double(I_Cb).*double(masks);
%     I_Cr128_1 = double(I_Cr).*double(masks);
    I_CbNew = sum((I_Cb.*masks)./repmat(sum(masks,3),[1 1 NumberOfImages]),3);
    I_CrNew = sum((I_Cr.*masks)./repmat(sum(masks,3),[1 1 NumberOfImages]),3);
...
if CG == 1 %if image is one channel (i.e., in grayscale representation)
    ...
    % Luminance and Chrominance combination for color images 
