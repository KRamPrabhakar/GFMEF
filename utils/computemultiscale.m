...
pyramid_levels  =  ceil( log(min(size(src, 1), size(src,2))/32) ...
    / log(pyramid_spacing) ); 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% 
% % extract SIFT
% [sift1, bbox1] = ExtractSIFT(pyramidRef{pyramid_levels}, pca_basis, sift_size);
% [sift2, bbox2] = ExtractSIFT(pyramidSrc{pyramid_levels}, pca_basis, sift_size);
% [uv(:,:,1), uv(:,:,2)] = DSPMatch(sift1, sift2);
% uv          = int32(uv);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for iLevel = pyramid_levels:-1:1
    ...
    % ppval - evaluates polynomial function ppIMF at location 0.5 (128)
    ...
    % for every zero pixels; compute the euclidian distance to the nearest
    % non-zero pixel location
    D = double(bwdist(mask));   
    dist_weight = (1.3*ones(size(D))).^(-D);
    % compare each element of dist_weight aganist 1.3^(-20)
    ...
    uvPrior(:,:,1:2) = uv + Z;
    uvPrior(:,:,3)   = patchdist(im2uint8(latentImg), pyramidSrc{iLevel}, int32(uv) + Z,  [], patchSize);
